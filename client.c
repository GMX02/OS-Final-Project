...
// Function to check if a username is profane
int isProfane(const char *username) {
    FILE *file = fopen("Profanity.txt", "r");
    if (file == NULL) {
        printf("Could not open Profanity.txt\n");
        return 0; // If we can't open the file, assume the username is acceptable
    }

    char line[100]; // Assuming words in Profanity.txt are less than 100 chars
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (len > 0 && (line[len - 1] == '\r')) { // Handle CRLF
            line[len - 1] = '\0';
            len--;
        }
        // Compare line with username, case-insensitive
        if (_stricmp(username, line) == 0) {
            fclose(file);
            return 1; // Username is profane
        }
    }
    fclose(file);
    return 0; // Username is acceptable
}
...
int main() {
    ...
    while (!usernameValid) {
        ...
        if (fgets(username, USERNAME_SIZE, stdin) != NULL) {
            ...
            // Check if the username is profane
            if (isProfane(username)) {
                printf("That username is not allowed, please try again.\n");
            } else {
                usernameValid = 1;
            }
        } else {
